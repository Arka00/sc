    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          m.isCommand = true
          let xp = 'exp' in plugin ? parseInt(plugin.exp) : 500 // XP Earning per command
          if (xp > 1000) m.reply('Ngecit -_-') // Hehehe
          else m.exp += xp
      ...
      case 'remove':
        if (chat.welcome) {
            ...
            for (let user of participants) {
              let kai = await(await fetch('https://telegra.ph/file/badb018177664ea8d3116.jpg ')).buffer()
              let poi = await(await fetch('https://telegra.ph/file/badb018177664ea8d3116.jpg')).buffer()
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'ようこそ Youkuso, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                ...
                mentionedJid: [user],
                externalAdReply: {
                  mediaUrl: 'https://github.com/Nanzz681',
...
global.dfail = (type, m, conn) => {
  ...
  if (unreg) return conn.sendButton(m.chat, `Halo kak ${name} Sebelum menggunakan fitur ini Harap Verify Terlebih dahulu`, watermark, 'Verify', `#daftar ${name}.17`, { key: { fromMe: false, remoteJid: 'status@broadcast', participant: '0@s.whatsapp.net' }, message: { orderMessage: { message: `© Nanzz_\nJam ${time}`, itemCount: 2022, thumbnail: fs.readFileSync('./src/RadBotZ.jpg')}}})
}
