...
let handler = async (m, { conn, text, usedPrefix }) => {
    ...
    m.reply(`Berhasil join grup ${res.gid} *BOT AKAN KELUAR DALAM WAKTU 2 MINGGU*`).then(() => {
        var jumlahHari = 1209600 * 0.2
        var now = new Date() * 1
...
ketik *#menu* untuk melihat daftar perintah`, MessageType.text, { contextInfo: { externalAdReply :{
       ...
       thumbnailUrl: image,
sourceUrl: 'https://wa.me/6282140059454?text=Assalamualaikum'
}}})
}
handler.help = ['join2 <linkgrup> ']
handler.tags = ['main']
